...
class TestVotes(APITestCase):
    ...
    url = "/compositions/{0}/votes"

    def createVote(self):
        composition = Helpers.createComposition()
        return Vote.objects.get(composition=composition)
    ...
    def createResponse(self, composition_id, positive, negative):
        return {'positive': positive, 'negative': negative, 'composition': composition_id}

    def refreshFromDB(self, vote):
        return Vote.objects.get(id=vote.id)
    ...
    def test_get_returns_votes_for_non_autheticated_user(self):
        ...
        url = self.url.format(vote.composition.id)

        response = self.client.get(url)

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data, self.createResponse(vote.composition.id, 1, 1))
    ...
    def test_get_returns_votes_for_autheticated_user(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data, self.createResponse(vote.composition.id, 2, 0))
        vote = self.refreshFromDB(vote)
    ...
    def test_post_works_for_authenticated_user_that_has_not_already_voted(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data, self.createResponse(vote.composition.id, 1, 0))
        vote = self.refreshFromDB(vote)
        self.assertEqual(vote.positive, 1)
        self.assertEqual(vote.negative, 0)
    ...
    def test_post_works_for_multiple_authenticated_user_that_has_not_already_voted(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data, self.createResponse(vote.composition.id, 2, 1))
        vote = self.refreshFromDB(vote)
        self.assertEqual(vote.positive, 2)
        self.assertEqual(vote.negative, 1)
