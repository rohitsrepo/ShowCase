...
class AdmirationsList(APIView):
    ...
    def get(self, request, format=None):
        user_admirations = request.user.admiration.all().order_by('-created')
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            if (serializer.data['admire_type'] == Admiration.ART):
                ...
                admire_type = Admiration.ART
            elif (serializer.data['admire_type'] == Admiration.BUCKET):
    ...
    def delete(self, request, format=None):
        serializer = AdmirationContentCreateSerializer(data=request.DATA, context={'request': request})
        if serializer.is_valid():
            admire_type = serializer.data['admire_type']

            if (admire_type == Admiration.ART):
                ctype = ContentType.objects.get(app_label='compositions', model='composition')
            elif (admire_type == Admiration.BUCKET):
                ctype = ContentType.objects.get(app_label='buckets', model='bucket')
            else:
                return Response(status=status.HTTP_400_BAD_REQUEST)
            ...
            try:
                Admiration.objects.get(owner=request.user,
                    ...
                    content_type=ctype,
                    admire_type=admire_type).delete()
            except:
                pass
...
class AdmirationsListRead(APIView):
    ...
    def get(self, request, user_id, format=None):
        ...
        user_admirations = user.admiration.all().order_by('-created')
