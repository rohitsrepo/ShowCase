...
from .models import StaffPost, FreshPost
...
def get_this_page_posts(paginator, page_num):
    try:
...
@api_view(['GET'])
@permission_classes((permissions.AllowAny,))
def fresh_list(request, format=None):
    page_num = request.GET.get('page', 1)

    fresh_posts = FreshPost.objects.all()
    paginator = Paginator(fresh_posts, settings.POSTS_PER_PAGE)
