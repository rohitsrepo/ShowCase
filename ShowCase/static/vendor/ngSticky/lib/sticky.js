	...
	module.directive('sticky', function() {
		...
		function linkFn($scope, $elem, $attrs) {
			...
			$scope.$watch( function() { // triggered on load and on digest cycle
				...
				parent_bot = _getTopOffset(elem.parentElement) + elem.parentElement.clientHeight;
			...
			function stickElement() {
				...
				var relativeLeft = absoluteLeft - $elem[0].parentElement.getBoundingClientRect().left;
			...
			function unstickElement() {
				...
				$elem
					.css('width',      '')
					.css('top',        (window.pageYOffset || doc.scrollTop)  - (doc.clientTop || 0) + offset - _getTopOffset(elem.parentElement))
					.css('position',   'absolute')
					.css('left',       initialCSS.relativeLeft)
			...
			function _getCurrentBottom (element) {
